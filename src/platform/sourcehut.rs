...
use gh_config::{is_enterprise, retrieve_token_from_env, retrieve_token_secure, Hosts, SOURCE_HUT_COM};
...
impl PlatformInit for SourceHut {
    type Config = Config;
    ...
    fn init(config: &Config) -> Result<Self> {
        ...
        let token = match token {
            Some(t) => t,
            _ => bail!("SourceHut access token could not be found. Install the gh CLI and login, or provide an token as GH_TOKEN environment variable."),
        };

        let mut builder = Octocrab::builder().personal_token(token);
        if config.host != SOURCE_HUT_COM {
...
impl Platform for SourceHut {}

#[async_trait]
impl Fork for SourceHut {
    async fn fork(&self, url: &Url, owner: Option<String>) -> Result<String> {
        ...
        Ok(request
            ...
            .html_url
            .as_ref()
            .ok_or_else(|| anyhow!("SourceHut API did not return HTML URL for the repository."))?
            .to_string())
    }
}

#[async_trait]
impl Browse for SourceHut {
